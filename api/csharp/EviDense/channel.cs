...
public class Channel(double sample = 0.0, double reference = 0.0)
{
    /// <summary>
    /// Gets or sets the sample value in uV.
    /// </summary>
    public double Sample { get; set; } = sample;

    /// <summary>
    /// Gets or sets the reference value in uV.
    /// </summary>
    public double Reference { get; set; } = reference;
    ...
    public JsonNode ToJson()
    {
        JsonObject obj = new()
        {
            [Dict.SAMPLE] = JsonValue.Create(Sample),
            [Dict.REFERENCE] = JsonValue.Create(Reference)
        };
