...
public class AdcResult(int sampleValue, int referenceValue, int vref, int current)
{
    /// <summary>
    /// Voltage of sample detector in uV.
    /// </summary>
    public int SampleValue { get; set; } = sampleValue;

    /// <summary>
    /// Voltage of reference detector in uV.
    /// </summary>
    public int ReferenceValue { get; set; } = referenceValue;

    /// <summary>
    /// Voltage of internal 5V reference in uV.
    /// </summary>
    public int Vref { get; set; } = vref;
...
public class LevellingChannelResult(int result, int current, int amplificationSample, int amplificationReference)
{
    /// <summary>
    ...
    /// 2: factor 111.0
    /// </remarks>
    public int AmplificationSample { get; set; } = amplificationSample;
    ...
    /// 2: factor 111.0
    /// </remarks>
    public int AmplificationReference { get; set; } = amplificationReference;
    ...
    public JsonNode ToJson()
    {
        JsonObject obj = new()
        {
            [Dict.LEVELLING_RESULT] = Result,
            [Dict.LEVELLING_CURRENT] = Current,
            [Dict.LEVELLING_AMPLIFIERSAMPLE] = AmplificationSample,
            [Dict.LEVELLING_AMPLIFIERREFERENCE] = AmplificationReference
        };
...
public class LevellingResult(LevellingChannelResult channel230, LevellingChannelResult channel260, LevellingChannelResult channel280, LevellingChannelResult channel340)
{
    /// <summary>
    /// Gets or sets the levelling result for the 230nm measurement channel.
    /// </summary>
    public LevellingChannelResult Channel230 { get; set; } = channel230;

    /// <summary>
    /// Gets or sets the levelling result for the 260nm measurement channel.
    /// </summary>
    public LevellingChannelResult Channel260 { get; set; } = channel260;

    /// <summary>
    /// Gets or sets the levelling result for the 280nm measurement channel.
    /// </summary>
    public LevellingChannelResult Channel280 { get; set; } = channel280;

    /// <summary>
    /// Gets or sets the levelling result for the 340nm measurement channel.
    /// </summary>
    public LevellingChannelResult Channel340 { get; set; } = channel340;
    ...
    public JsonNode ToJson()
    {
        JsonObject obj = new()
        {
            [Dict.CH_230] = Channel230.ToJson(),
            [Dict.CH_260] = Channel260.ToJson(),
            [Dict.CH_280] = Channel280.ToJson(),
            [Dict.CH_340] = Channel340.ToJson()
        };
...
public class SelfTestResult(int result)
{
    /// <summary>
...
public class Device : IDisposable
{
    ...
    private readonly bool isSimulation_;
    ...
    public Device(string? serialNumber = null)
    {
        ...
        else
        {
            ...
            var serialPortName = DeviceFinder.FindDevice(serialNumber) ?? throw new Exception($"EviDense ({serialNumber}) module not found");
    ...
    public bool IsSimulation => isSimulation_;
    ...
    public string[] Command(string tx)
    {
        ...
        string[] parts = Split(rx[1..]);

        if (parts[0] == "E")
        {
            if (parts.Length >= 2)
                if (int.TryParse(parts[1], out int errorCode))
    ...
    private static string? XferSocket(string tx)
    {
    ...
    private static string ErrorToText(int error)
    {
        ...
        {
            (int)Error.OK => "OK",
            (int)Error.UNKNOWN_COMMAND => "Unknown command",
            (int)Error.INVALID_PARAMETER => "Invalid parameter",
            (int)Error.SREC_FLASH_WRITE_ERROR => "Invalid parameter",
            (int)Error.SREC_UNSUPPORTED_TYPE => "SREC: unsupported type",
            (int)Error.SREC_INVALID_CRC => "SREC: invalid crc",
            (int)Error.SREC_INVALID_STRING => "SREC: invalid string",
            (int)Error.LEVELLING_FAILED => "Levelling failed",
            _ => "Unknown error",
        };
    }
    ...
    public void FwUpdate(string[] lines, UpdateProgress? progress = null, Rebooting? rebooting = null)
    {
        ...
        rebooting?.Invoke(true);
        Reboot();
        serialPort_?.Close();

        Stopwatch stopWatchDelay = new();
    ...
    public void AddSelfTestDetails(JsonNode? node)
    {
        JsonNode LedAsJson(Index ILed, Index DarkSample, Index DarkReference, Index Sample, Index Reference)
        {
            JsonObject obj = new()
            {
                [Dict.SELFTEST_ILED] = Get<int>(ILed),
                [Dict.SELFTEST_DARKSAMPLE] = Get<int>(DarkSample),
                [Dict.SELFTEST_DARKREFERENCE] = Get<int>(DarkReference),
                [Dict.SELFTEST_SAMPLE] = Get<int>(Sample),
                [Dict.SELFTEST_REFERENCE] = Get<int>(Reference)
            };
    ...
    public JsonNode TechnicalReport()
    {
        JsonObject obj = new()
        {
            [Dict.LEVELLING] = Levelling().ToJson(),
            [Dict.SELFTEST] = SelfTest().ToJson()
        };
